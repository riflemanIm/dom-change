...
import { CatalogAmenity, getCatalog, getCatalogAmenities } from '@/features/catalog/catalog-api';
...
export default async function HomesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  try {
    [result, amenities] = await Promise.all([getCatalog(params), getCatalogAmenities()]);
  } catch (reason) {
    return (
      <><Header /><Box component="main" sx={{ bgcolor: '#f8faf8', minHeight: '70vh' }}><Container maxWidth="xl" sx={{ py: 6 }}><Typography variant="h2">Каталог жилья</Typography><CatalogFilters initial={search} amenities={[]} /><Alert severity="error">{(reason as Error).message}</Alert></Container></Box><Footer /></>
    );
  }
  ...
  return (
    <><Header /><Box component="main" sx={{ bgcolor: '#f8faf8', minHeight: '70vh', py: { xs: 4, md: 6 } }}><Container maxWidth="xl">
      ...
      <CatalogFilters key={params.toString()} initial={search} amenities={amenities} />
