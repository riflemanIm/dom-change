...
export default async function HomesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  try {
    ...
    return (
      <><Header /><Box component="main" sx={{ bgcolor: '#f8faf8', minHeight: '70vh' }}><Container maxWidth="xl" sx={{ py: 6 }}><Typography variant="h2">Каталог жилья</Typography><CatalogFilters initial={search} /><Alert severity="error">{(reason as Error).message}</Alert></Container></Box><Footer /></>
    );
  }
